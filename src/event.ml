...
let%expect_test _ =
  let columns =
    ...
    [ c "id" (fun (event : t) -> Int.to_string event.id)
    ; c "short" (fun (event : t) -> event.short)
    ; c "precise" (fun (event : t) -> event.precise)
    ; c "outcome" (fun (event : t) -> Outcome.to_string event.outcome)
    ]
  in
  print_endline
