open! Core
module Bonsai = Bonsai.Cont
open Bonsai.Let_syntax
open Bonsai_web.Cont
open Crystal
open Crystal_bonsai

let responses_and_scores =
  [%blob "./etc/2026/responses_and_scores.sexp"]
  |> Sexp.of_string
  |> [%of_sexp: Responses_and_scores.t String.Map.t]
;;

let all graph =
  let plots = Plots.create ~events:Crystal_2026.all ~responses_and_scores in
  let scores = Map.map responses_and_scores ~f:Responses_and_scores.scores in
  let standings =
    Standings.component
      ~start_date:(Date.of_string "2026-01-01")
      ~end_date:(Date.of_string "2026-12-31")
      Crystal_2026.all
      scores
      graph
  in
  let plots = Plots.component plots graph in
  let explorer =
    Explorer.component
      ~db_path:"./crystal.db"
      ~initial_query:
        {|SELECT
  name, sql
FROM
  sqlite_master
WHERE
  type IN ('table', 'view')|}
      graph
  in
  let%arr standings = standings
  and plots = plots
  and explorer = explorer in
  let open Vdom in
  Node.div
    [ Node.h2 [ Node.text "Standings" ]
    ; standings
    ; Node.h2 [ Node.text "Events" ]
    ; plots
    ; Node.h2 [ Node.text "Explorer" ]
    ; explorer
    ]
;;

let () = Bonsai_web.Start.start ~bind_to_element_with_id:"app" all
