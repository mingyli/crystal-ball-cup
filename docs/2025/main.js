...
function renderTable(questionId) {
    ...
    const scores = standings.map(s => s.meanTotalScore);
    ...
    const data = [{
        ...
        marker: {
            color: scores.map(score => score >= 0 ? GREEN_FILL : RED_FILL),
            line: {
                color: scores.map(score => score >= 0 ? GREEN : RED),
                width: 1
            }
        }
    }];
    ...
    const layout = {
        ...
        height: 20 * users.length + 80, // Dynamic height
    };

    Plotly.newPlot(container, data, layout, {displayModeBar: false});

    // The sorting functionality is now part of the bar chart rendering,
    // so we don't need the old sortTable function.
    // The dropdowns for sorting will also be removed.
}
