...
function renderStandings(responsesAndScores) {
  ...
  const maxAbsScore =
    finiteScores.length > 0
      ? Math.max(...finiteScores.map((s) => Math.abs(s)))
      : 1;
...
function renderEverything(events, responsesAndScores) {
  ...
  const attachClickHandler = (plotContainerId) => {
    document
      .getElementById(plotContainerId)
  ...
  const renderPlot = (
    ...
    if (plotType === "density") {
      ...
      const trace2 = createScatterTrace(
        eventData,
        Array(eventData.length).fill(" "),
        allRespondents,
        highlightedRespondent,
        responsesAndScores,
        event.id,
    ...
    Plotly.newPlot(plotContainer.attr("id"), traces, layout, {
  ...
  const plotData = (eventId, highlightedRespondent, plotType) => {
    ...
    const eventsToPlot =
      eventId === "all" ? events : events.filter((e) => e.id == eventId);
